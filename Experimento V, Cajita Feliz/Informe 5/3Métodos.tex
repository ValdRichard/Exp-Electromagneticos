...
Se estudió un circuito RC de distribución desconcida, con el objetivo de modelar su composición en función de su comportamiento, para ello se realizaron diferentes experimentos. El arreglo experimental (\autoref{fig:arreglo}) consistió en un una fuente de tensión alterna conectada en serie a una caja negra cuya composición se quiere conocer y a una resistencia sobre la cual se midió la señal de salida. La señal de entrada y la señal de salida se registraron simultáneamente utilizando un osciloscopio.

\begin{figure} [H]
    \centering
    \includegraphics[width=0.5\linewidth]{Figuras/esquema.png}
...
Un experimento consisitió en variar la frecuencia de la señal sinusoidal de entrada, dada por un generador de ondas $Sinometer$ modelo VC2002 \cite{VC2002} o un generador de ondas $OWON$ modelo DG31000 \cite{DGE1000}, manteniendo la amplitud constante y variando la frecuencia $f$ en el rango [0,01; 60] \SI{}{\kilo\hertz}. La señal de entrada $V_{in}$ y la señal de salida $V_{out}$, se registraron simultáneamente utilizando un osciloscopio $Tektronix$ modelo TDS 220 \cite{TDS200}. A partir de estos datos se calculó la impedancia del sistema como el cociente entre las amplitudes de entrada y salida. Además en cada medición se registró la diferencia temporal $\Delta t$ entre ellas. A partir de estos datos se calculó el respectivo desfasaje mediante la relación $\varphi=2\pi f \Delta t$, donde $f$ es la frecuencia de $V_{in}$ y $\omega=2\pi f$. 
